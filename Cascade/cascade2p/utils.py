...
def define_model(filter_sizes, filter_numbers, dense_expansion, windowsize, loss_function, optimizer):
    ...
    """
    inputs = tf.keras.layers.Input(shape=(windowsize, 1))
    ...
    outX = tf.keras.layers.MaxPooling1D(2)(outX)
    outX = conv_filter(filter_numbers[2], filter_sizes[2], activation='relu')(outX)
    outX = tf.keras.layers.MaxPooling1D(2)(outX)

    outX = tf.keras.layers.Dense(dense_expansion, activation='relu')(outX)
    outX = tf.keras.layers.Flatten()(outX)
    predictions = tf.keras.layers.Dense(1, activation='linear')(outX)
