...
def save_section_comparisons(
    ...
    """Write aggregate section tables and plots at every tree node."""
    for node in root.iter_nodes():
        comparison_dfs = node.section_comparison_dfs
        ...
        for strategy_name, strategy_dfs in comparison_dfs.items():
            for section_key, comparison in strategy_dfs.items():
                path = (
                    out_dir
                    / (
                        f"{node.name}_{strategy_name}_{section_key}"
                        "_section_comparison.csv"
                    )
                )
                comparison.to_csv(path, index=False)

        for strategy_name, strategy_metrics in (
            node.section_comparison_metrics.items()
        ):
            for section_key, metrics in strategy_metrics.items():
                if not metrics:
                    continue
                title = f"{node.name} - {strategy_name} - {section_key}"

                delta_figure, delta_axis = plt.subplots(figsize=(7, 5))
                plot_delta_corr_vs_dispersion(
                    metrics,
                    ax=delta_axis,
                    title=title,
                ...
                centroid_figure, centroid_axis = plt.subplots(figsize=(7, 6))
                plot_neuron_centroid_distances(
                    metrics,
                    ax=centroid_axis,
                    title=title,
                )
                centroid_figure.tight_layout()
                centroid_figure.savefig(
                    out_dir
                    / (
                        f"{node.name}_{strategy_name}_{section_key}"
                        "_centroid_distances.png"
                    ),
                    dpi=150,
                    bbox_inches="tight",
                )
