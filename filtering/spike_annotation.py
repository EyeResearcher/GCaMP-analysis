...
def label_spikes(sampled: pd.DataFrame):
    ...
    for i, row in sampled.iterrows():
        spike = row["spk_instance"]
        suite2p_path = row['suite2p_path']
        raw_f = np.load(suite2p_path / 'F.npy')[spike.roi_index]
        spike_prob = np.load(suite2p_path / 'cascade_spike_prob.npy')[spike]
...
def main_annotate(features_df: pd.DataFrame):
    if "spk_instance" not in features_df.columns:
        features_df = create_spike_instances(features_df)
    n_annotations = int(input("How many spikes do you want to annotate? "))
