...
def parse_arguments():
    ...
    parser.add_argument('--viz_features', action='store_true', help='Visualize features with PCA and pairwise scatter')
...
def main():
    ...
    if args.new_spikes:
        ...
        reinst_df, feat_df = spike_dataset_feature_computation(dataset_root, roi_labels_path, model_name=model_name, new_model=new_model)
        feat_df.to_csv(model_version_folder / 'spike_filtering' / 'spike_features.csv', index=False)
        reinst_df.to_csv(model_version_folder / 'spike_filtering' / 'spike_reinstantiation.csv', index=False)
        print(f"Computed features for {len(feat_df)} spikes.")
        ...
        reinst_df = pd.read_csv(model_version_folder / 'spike_filtering' / 'spike_reinstantiation.csv')
        reinst_df, features_df_new = features_only(reinst_df)
    ...
    if args.viz_features:
        from .spike_visualize import plot_feature_distributions
        features_df = pd.read_csv(model_version_folder / 'spike_filtering' / 'spike_features.csv')
        annotations_df = pd.read_csv(model_version_folder / 'spike_filtering' / 'spike_annotations.csv')
        reinst_df = pd.read_csv(model_version_folder / 'spike_filtering' / 'spike_reinstantiation.csv')
        merged_df = pd.merge(annotations_df, features_df, on='spike_key', how='left')
        stats = plot_feature_distributions(merged_df)
